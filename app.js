...
const CONFIG = {
  ...
  LABEL_MIN_ZOOM: 13,
  supabase: {
    url: "https://maysjvwieljjomlqmgem.supabase.co",
    key: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1heXNqdndpZWxqam9tbHFtZ2VtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA0MDA4NTEsImV4cCI6MjA5NTk3Njg1MX0.Em3h7hG7itOPJuB3EPZbvxPlWj0golr6fTh2243CKSE",
    table: "catastro",
...
// Catálogo de especies y su campo de cantidad en la BD
const SPECIES_OPTIONS = [
  { name: "Pacú", norm: "pacu", qty: "Cuanto_Pac" },
  { name: "Tambaqui", norm: "tambaqui", qty: "Cuanto_Tam" },
  { name: "Surubí", norm: "surubi", qty: "Cuanto_Sur" },
  { name: "Sabalo", norm: "sabalo", qty: "Cuanto_Sab" },
  { name: "Tilapia", norm: "tilapia", qty: "Cuanto_Til" },
  { name: "Blanquillo", norm: "blanquillo", qty: "Cuanto_Bla" },
  { name: "Pangacio", norm: "pangacio", qty: "Cuanto_Pan" },
];
...
let layerCatastroVacios = null;       // predios vacíos clickeables (para registrar datos)
...
function styleCatastroVacio() {
  // Predios sin datos: borde claro + relleno casi invisible para que sean clickeables
  return {
    color: "rgba(239,255,255,.55)",
    weight: 0.9,
    opacity: 0.7,
    fillColor: "#9fb6bd",
    fillOpacity: 0.04,
...
function buildCatastroPopup(feature) {
  ...
  const SPEC_Q_FIELDS = [
    ...
    { species: "Sabalo", field: "Cuanto_Sab", label: "Cuanto Sabalo produjo?" },
    { species: "Tilapia", field: "Cuanto_Til", label: "Cuanta Tilapia produjo?" },
    { species: "Blanquillo", field: "Cuanto_Bla", label: "Cuanto Blanquillo produjo?" },
  ...
  if (!Number.isNaN(id) && id > 0) {
    const photoBase = `${CONFIG.supabase.url}/storage/v1/object/public/${CONFIG.supabase.storageBucket}`;
    const jpg = `${photoBase}/${id}.jpg`;
    const png = `${photoBase}/${id}.png`;
    photoBlock = `
      <div class="popup-photo">
...
window.handleEditClick = function (uid) {
  ...
  if (content && feat) {
    content.innerHTML = renderEditForm(feat.properties);
    content.scrollTop = 0;
  }
};

// Bloque de selección múltiple de especies + cantidades condicionales
function renderSpeciesBlock(p) {
  const current = parseSpeciesList(safeVal(p["Que_especi"] ?? p["QUE_ESPECI"] ?? ""))
    .map(normalizeSpeciesName);

  const items = SPECIES_OPTIONS.map(sp => {
    const checked = current.includes(sp.norm);
    const qtyVal = safeVal(p[sp.qty] ?? "");
    return `
      <div class="species-item">
        <label class="species-check">
          <input type="checkbox" class="species-box" data-norm="${sp.norm}"
                 data-name="${sp.name}" data-qty="${sp.qty}" ${checked ? "checked" : ""}
                 onchange="window.toggleSpeciesQty(this)">
          <span>${sp.name}</span>
        </label>
        <div class="species-qty ${checked ? "" : "hidden"}" data-qty-for="${sp.norm}">
          <label>¿Cuánto ${sp.name} produjo? (kg)</label>
          <input type="text" name="${sp.qty}" class="edit-input" inputmode="numeric" value="${qtyVal}">
        </div>
      </div>
    `;
  }).join("");
  ...
  return `
    <tr><td colspan="2" style="padding:0;">
      <div class="species-block">
        <div class="species-block__title">¿Qué especies cultiva?</div>
        ${items}
      </div>
    </td></tr>
  `;
...
window.toggleSpeciesQty = function (box) {
  const norm = box.getAttribute("data-norm");
  const wrap = document.querySelector(`.species-qty[data-qty-for="${norm}"]`);
  if (!wrap) return;
  wrap.classList.toggle("hidden", !box.checked);
  const inp = wrap.querySelector("input");
  if (inp) {
    if (box.checked) { inp.focus(); }
    else { inp.value = "0"; }
  }
};
...
window.handleCancelEdit = function (uid) {
  ...
  const content = document.getElementById("feature-panel-content");
  if (content && feat) {
    const isEmpty = getEstadoRaw(feat.properties) === "";
    content.innerHTML = isEmpty ? buildVacioPopup(feat) : buildCatastroPopup(feat);
    content.scrollTop = 0;
  }
};
...
window.handleSave = async function (event, uid) {
  ...
  const updateData = {};
  const numericFields = ["Estanques", "Est_Obser", "Nro_Bombas",
    "Cuanto_Tam", "Cuanto_Pac", "Cuanto_Sur", "Cuanto_Sab",
    "Cuanto_Til", "Cuanto_Bla", "Cuanto_Pan", "ID_12"];
  ...
  // Al editar/registrar datos, el predio se considera Completo (se mantiene para
  // la visualización en el mapa y los indicadores del dashboard).
  updateData["Estado"] = "Completo";

  // Especies (checkboxes): construir Que_especi y poner en 0 las cantidades no marcadas
  const speciesBoxes = Array.from(form.querySelectorAll(".species-box"));
  if (speciesBoxes.length) {
    const checkedNames = speciesBoxes.filter(b => b.checked).map(b => b.getAttribute("data-name"));
    updateData["Que_especi"] = checkedNames.join(", ");
    speciesBoxes.forEach(b => {
      if (!b.checked) updateData[b.getAttribute("data-qty")] = 0;
    });
  }
  ...
  try {
    ...
    if (file) {
      ...
      // Reusar el ID_12 existente o generar uno nuevo (timestamp) para predios sin foto
      const newId12 = id12 || Date.now();
      const fileName = `${newId12}.jpg`;
      ...
      // Guardar el ID de la foto en los datos nuevos para que se muestre en
      // el panel admin y se actualice el predio al aprobar.
      updateData.ID_12 = newId12;
    }
...
function onEachCatastro(feature, layer) {
  layer.on("click", () => openFeaturePanel(feature, layer));
}

// Popup para predios SIN datos (vacíos): permite registrar al productor
function buildVacioPopup(feature) {
  const p = feature.properties || {};
  const sind = safeVal(getCatastroSindicacto(p));
  const central = safeVal(getCatastroCentral(p));

  return `
    <div class="popup-wrap">
      <div class="popup-header">Predio sin registro</div>
      <div class="popup-scroll">
        <p style="font-size:13px; color:#bcd; margin:8px 0 10px; line-height:1.4;">
          Este predio aún no tiene datos de catastro acuícola. Si se construyeron
          estanques aquí, registre los datos del productor.
        </p>
        <table class="popup-table">
          <tr><td class="popup-k">Sindicato</td><td class="popup-v">${sind || "-"}</td></tr>
          <tr><td class="popup-k">Central</td><td class="popup-v">${central || "-"}</td></tr>
        </table>
      </div>
      <div class="popup-actions" style="margin-top: 10px; display: flex; gap: 8px;">
        <button class="btn-edit-feature" style="flex: 1; padding: 8px; cursor: pointer; background: var(--c5); color: white; border: none; border-radius: 6px;"
                onclick='event.stopPropagation(); handleEditClick(${p._uid})'>
          <i class="fa-solid fa-plus"></i> Registrar datos del productor
        </button>
      </div>
    </div>
  `;
...
function onEachCatastroVacio(feature, layer) {
  layer.on("click", () => openFeaturePanel(feature, layer));
  // Realce al pasar el mouse para indicar que es clickeable (sin pisar el seleccionado)
  layer.on("mouseover", () => {
    if (layer !== selectedFeatureLayer) layer.setStyle({ fillOpacity: 0.20, color: "#56ff9d", weight: 1.4 });
  });
  layer.on("mouseout", () => {
    if (layer !== selectedFeatureLayer) layer.setStyle(styleCatastroVacio());
  });
}

// =======================================================
// PANEL LATERAL DE DETALLE + RESALTADO EN AMARILLO
// =======================================================
...
function highlightFeatureLayer(layer) {
  ...
  layer.setStyle({ color: "#ffd400", weight: 3, fillColor: "#ffe24d", fillOpacity: 0.5 });
  if (layer.bringToFront) layer.bringToFront();
}

function clearFeatureHighlight() {
  if (selectedFeatureLayer && selectedFeatureLayer.setStyle) {
    const feat = selectedFeatureLayer.feature;
    const empty = getEstadoRaw(feat?.properties || {}) === "";
    try {
      selectedFeatureLayer.setStyle(empty ? styleCatastroVacio() : styleCatastro(feat));
    } catch (_) { /* capa removida */ }
  }
  selectedFeatureLayer = null;
}

function openFeaturePanel(feature, layer) {
  const content = document.getElementById("feature-panel-content");
  const panel = document.getElementById("feature-panel");
  if (!content || !panel) return;

  const empty = getEstadoRaw(feature.properties) === "";
  content.innerHTML = empty ? buildVacioPopup(feature) : buildCatastroPopup(feature);
  content.scrollTop = 0;

  const titleEl = panel.querySelector(".feature-panel__title");
  if (titleEl) {
    titleEl.textContent = empty
      ? "Predio sin registro"
      : (safeVal(feature.properties.Nombre) || "Detalle del predio");
  }
...
window.closeFeaturePanel = closeFeaturePanel;
...
function getOverlayState() {
  return {
    ...
    catastroVacios: layerCatastroVacios ? map.hasLayer(layerCatastroVacios) : false,
...
function applyOverlayState(state) {
  ...
  if (state.catastroVacios) map.addLayer(layerCatastroVacios); else map.removeLayer(layerCatastroVacios);
...
function renderLegend() {
  ...
  addIfActive("Predios sin registro", layerCatastroVacios, `
    <div class="legend-row">
      <span class="legend-swatch" style="background:#9fb6bd; opacity:.5; border:1px solid rgba(239,255,255,.55)"></span>
      Sin datos (clic para registrar)
    </div>
  `);
...
function flyToFeature(feature, openPopup = true) {
  ...
  // Identificar el predio por su id (o _uid) en las capas interactivas
  const targetId = feature.properties?.id ?? feature.properties?._uid;
  if (targetId === undefined || targetId === null) return;

  let found = null;
  [layerCatastroDatos, layerCatastroVacios].forEach(layer => {
    if (!layer) return;
    layer.eachLayer(l => {
      const p = l.feature?.properties || {};
      const pid = p.id ?? p._uid;
      if (pid === targetId) found = l;
    });
  });

  if (found) openFeaturePanel(found.feature, found);
}
...
function rebuildFilteredLayers() {
  ...
  // Las capas se recrean: la referencia al predio resaltado deja de ser válida
  selectedFeatureLayer = null;

  const prevState = getOverlayState();

  // remover actuales si estaban en mapa
  [layerCatastroDatos, layerCatastroVacios, layerSindicatos, layerSindicatosLabels, layerEstanquesFiltered].forEach(l => {
  ...
  // catastro VACÍO (Estado vacío) -> clickeable para registrar datos del productor
  layerCatastroVacios = L.geoJSON(catastroGJ_ALL, {
    style: styleCatastroVacio,
    filter: (f) => {
      const p = f.properties || {};
      if (getEstadoRaw(p) !== "") return false;   // solo predios sin datos
      const c = currentFilters.central;
      const s = currentFilters.sindicato;
      if (c && getCatastroCentral(p) !== c) return false;
      if (s && getCatastroSindicacto(p) !== s) return false;
    ...
    onEachFeature: onEachCatastroVacio
  });
  ...
  overlayMaps["Predios sin registro"] = layerCatastroVacios;
...
// Convierte geometría EWKB en hexadecimal (formato PostGIS) a GeoJSON.
...
function ewkbHexToGeoJSON(hex) {
  ...
  function readGeom() {
    const little = dv.getUint8(pos) === 1; pos += 1;
    let type = dv.getUint32(pos, little); pos += 4;
    const hasZ = (type & 0x80000000) !== 0;
    const hasM = (type & 0x40000000) !== 0;
    const hasSRID = (type & 0x20000000) !== 0;
    const base = type & 0xff;
    if (hasSRID) pos += 4; // saltar SRID

    function readPoint() {
      const x = dv.getFloat64(pos, little); pos += 8;
      const y = dv.getFloat64(pos, little); pos += 8;
      if (hasZ) pos += 8;
      if (hasM) pos += 8;
      return [x, y];
    }
    function readRing() {
      const n = dv.getUint32(pos, little); pos += 4;
      const ring = new Array(n);
      for (let i = 0; i < n; i++) ring[i] = readPoint();
      return ring;
    }
    function readPolygon() {
      const nr = dv.getUint32(pos, little); pos += 4;
      const rings = new Array(nr);
      for (let i = 0; i < nr; i++) rings[i] = readRing();
      return rings;
    }
    ...
    switch (base) {
      ...
      case 3: return { type: "Polygon", coordinates: readPolygon() };
      case 4: {
        const n = dv.getUint32(pos, little); pos += 4;
        const c = [];
        for (let i = 0; i < n; i++) c.push(readGeom().coordinates);
  ...
  try { return readGeom(); }
  catch (e) { console.warn("EWKB parse error:", e); return null; }
}
...
async function loadCatastroFromSupabase() {
  ...
  return {
    ...
    features: allData.map(item => {
      // Resolver geometría: puede venir como GeoJSON (objeto/string) o EWKB hex (PostGIS)
      ...
      if (!geometry && raw) {
        if (typeof raw === "object") {
          ...
          const g = raw.trim();
          if (g.startsWith("{")) {
            try { geometry = JSON.parse(g); } catch (_) { geometry = null; }
          } else {
            geometry = ewkbHexToGeoJSON(g); // WKB hexadecimal
          }
        }
      }

      // Quitar el geom crudo de las propiedades (evita arrastrar el hex enorme a las ediciones)
      const props = { ...item };
      delete props.geom;
...
(function () {
  ...
  function wireLoginModal() {
    ...
    if (btnLogin) {
      btnLogin.addEventListener("click", async () => {
        ...
        if (!email || !pwd) {
          if (errEl) { errEl.textContent = "Ingrese email y contraseña."; errEl.style.display = "block"; }
          return;
        }
        ...
        try {
          ...
          if (errEl) {
            errEl.textContent = err.message || "Credenciales incorrectas. Intente nuevamente.";
    ...
    // Enter en los campos también lanza el login
    ["admin-login-email", "admin-login-password"].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.addEventListener("keydown", e => { if (e.key === "Enter") document.getElementById("btn-admin-login")?.click(); });
    });
  ...
  async function adminLogout() {
    try { await supabaseClient.auth.signOut(); } catch (_) { /* ignorar */ }
  ...
  async function checkAdminAuth() {
    // Verificar si ya hay sesión activa en Supabase
    try {
  ...
  function renderChangeCard(item) {
    ...
    const compareFields = [
      ...
      ["Que_especi", "Especies"], ["Cuanto_Pac", "Cant. Pacú"],
      ["Cuanto_Tam", "Cant. Tambaqui"], ["Cuanto_Sur", "Cant. Surubí"],
      ["Cuanto_Sab", "Cant. Sabalo"], ["Cuanto_Til", "Cant. Tilapia"],
      ["Cuanto_Bla", "Cant. Blanquillo"], ["Cuanto_Pan", "Cant. Pangacio"],
      ["ID_12", "Foto ID"]
    ];
  ...
  async function handleAdminAction(changeId, newStatus) {
    ...
    try {
      ...
      const { error: finalErr } = await supabaseClient.from("catastro_ediciones").update({
        ...
        revisado_por: user?.email || "Administrador"
      }).eq("id", changeId);
